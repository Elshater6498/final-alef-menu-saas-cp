...
function Ratings() {
    ...
    const showRating = useAppSelector(
        (state) => state.restaurant.restaurant.showRates
    )

    const fetchRates = async () => {
        try {
            const response = await apiGetRates()
            const transformedData = transformRateToEvaluation(response.data)
    ...
    const columns = useMemo<ColumnDef<MenuEvaluation>[]>(
        () => [
            ...
            {
                header: `${t('ratings.name')}`,
                accessorKey: 'name',
            },
            {
                header: `${t('ratings.phone')}`,
                accessorKey: 'phone',
            },
            {
                header: `${t('ratings.email')}`,
                accessorKey: 'email',
            },
            {
                header: `${t('ratings.rate time')}`,
    ...
    const handleSubmit = async (value: boolean) => {
        ...
        formData.append('showRates', `${value}`)
    ...
    return (
        <>
            <Title Icon={MdOutlineStarRate} title={`${t('nav.ratings')}`} />
            ...
            >
                ...
                />
                <span>
                    {t('ratings.(hide / show )the rating from the side menu')}
                </span>
            </Tag>
