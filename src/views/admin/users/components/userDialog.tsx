...
const UsersDialog = (props: Props) => {
  ...
  const handleFormSubmit = async (values: ClientForm) => {
    ...
    for (let i = 0; i < values.permissions!.length; i++) {
      ...
      if(element == "users"){
        permissions.push(...permissionsList.users)
      }
      // if(element == "plans"){
      //   permissions.push(...permissionsList.plans)
      // }
    }
    if(props.selectedRow.name){
    ...
    const options = [
      {
        label: `${t('nav.client-management')}`,
        value: "users"
      },
      // {
      //   label:  `${t('nav.package-management')}`,
      //   value: "plans"
      // }
    ]
  ...
  return (
    ...
    >
      ...
      <Formik
        initialValues={props.selectedRow.name ?  {...props.selectedRow ,permissions: [...options.map((item) => {
          if (props.selectedRow.permissions?.includes('users.list')) {
            return item
          }
          else return
